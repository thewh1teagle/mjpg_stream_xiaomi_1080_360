...
void *client_thread(void *arg)
{
    ...
    //LUK: signal start capture
    pthread_mutex_lock(&pglobal->in[input_number].mnclients);
    pglobal->in[input_number].num_clients++;
    pthread_mutex_unlock(&pglobal->in[input_number].mnclients);
    //
    ...
    */
    ...
    //LUK: added   
    syslog(LOG_INFO, "Leaving HTTP client thread\n");

    //LUK: 
    pthread_mutex_lock(&pglobal->in[input_number].mnclients);
    if(pglobal->in[input_number].num_clients > 0)
      pglobal->in[input_number].num_clients--;
    pthread_mutex_unlock(&pglobal->in[input_number].mnclients);
    //
